/* ==========================================================
   ...
   - Compatível com código colado diretamente em IntranetUserJS
   ========================================================== */

(function () {
  'use strict';

  if (window.__authorityBoxRbmoIntranetLoaded) return;
  window.__authorityBoxRbmoIntranetLoaded = true;

  const CONFIG = {
    version: '1.0-intranet',
    maxAutoridades: 12,
    titulo: 'Autor(es)',
  ...
  function arrancarAuthorityBox() {
    setTimeout(initAuthorityBoxIntranet, 900);
  }
  ...
  async function initAuthorityBoxIntranet() {
    if (!isIntranetDetailPage()) return;

    const autores = await recolherAutores();
    ...
    if (principal && principal.autor) window.__authorityBoxRbmoPrimaryAuthor = principal.autor;
  ...
  function isIntranetDetailPage() {
    return window.location.pathname.includes('/cgi-bin/koha/catalogue/detail.pl') ||
      window.location.href.includes('/catalogue/detail.pl');
  }

  async function recolherAutores() {
    let autores = recolherAutoresVisiveis();

    if (!autores.length) {
      autores = await recolherAutoresDoMarc();
    }
  ...
  function recolherAutoresVisiveis() {
    const autores = [];
    const seletorLinks = [
      'a[href*="/authorities/detail.pl"][href*="authid="]',
      'a[href*="/authorities/authorities.pl"][href*="authid="]',
      'a[href*="catalogue/search.pl"][href*="q="]'
    ].join(',');

    document.querySelectorAll('.results_summary, tr, .page-section').forEach(function (bloco) {
      const label = obterLabelDoBloco(bloco);
      if (!CONFIG.camposValidos.includes(label)) return;

      const links = Array.from(bloco.querySelectorAll(seletorLinks));
  ...
  async function recolherAutoresDoMarc() {
    const biblionumber = new URLSearchParams(window.location.search).get('biblionumber');
    if (!biblionumber) return [];

    const urls = [
      '/cgi-bin/koha/catalogue/MARCdetail.pl?biblionumber=' + encodeURIComponent(biblionumber),
      '/cgi-bin/koha/catalogue/showmarc.pl?id=' + encodeURIComponent(biblionumber)
    ];
    ...
    for (const url of urls) {
      try {
        ...
        const autores = extrairAutoresMarcHtml(html);
        if (autores.length) return autores;
      } catch (e) {
        console.warn('AuthorityBox: erro ao ler MARC bibliográfico', url, e);
      }
    }
  ...
  function extrairAutoresMarcHtml(html) {
    ...
    if (!grupos.length) {
      const texto = limparTexto(doc.body ? doc.body.textContent : html);
      const re = /(?:^|\s)(700|701|702)\s+([\s\S]*?)(?=\s(?:[0-9]{3})\s|$)/g;
      let m;
      while ((m = re.exec(texto)) !== null) {
        grupos.push({ tag: m[1], html: '', texto: limparTexto(m[2]) });
      }
    }

    grupos.forEach(function (grupo) {
      const authid = extrairAuthIdMarc(grupo.html + ' ' + grupo.texto);
      if (!authid) return;

      const nome = extrairNomeMarc(grupo.html, grupo.texto);
      if (!nome) return;

      const label = grupo.tag === '700'
        ? 'autor'
        : (grupo.tag === '701' ? 'co-autor' : 'autor secundário');

      autores.push({
        nome: limparNomeAutor(nome),
        nomeOriginal: nome,
        href: '/cgi-bin/koha/authorities/detail.pl?authid=' + encodeURIComponent(authid),
        authid: authid,
        labelLinha: label,
        papeis: extrairPapeisDoTexto(nome, label)
      });
    });
  ...
  function extrairAuthIdMarc(texto) {
    const bruto = String(texto || '');

    let m =
      bruto.match(/[?&]authid=(\d+)/i) ||
      bruto.match(/(?:\$|_)9\s*(\d+)/i) ||
      bruto.match(/(?:subfield[^>]*>\s*9\s*<[^>]*>[\s\S]{0,150}?)(\d{1,12})/i);
    ...
    for (let i = 0; i < cells.length - 1; i++) {
      const label = limparTexto(cells[i].textContent).toLowerCase();
      const valor = limparTexto(cells[i + 1].textContent);
      if ((label === '9' || label === '$9' || label.endsWith(' 9')) && /^\d+$/.test(valor)) {
        return valor;
      }
    }
  ...
  function extrairNomeMarc(html, texto) {
    const doc = new DOMParser().parseFromString(String(html || ''), 'text/html');

    const authorityLink = doc.querySelector(
      'a[href*="/authorities/detail.pl"][href*="authid="], ' +
      'a[href*="/authorities/authorities.pl"][href*="authid="]'
    );
    if (authorityLink) {
      const nomeLink = limparTexto(authorityLink.textContent);
      if (nomeLink) return nomeLink;
    }

    const bruto = limparTexto(texto);
    const a = bruto.match(/(?:\$|_)a\s*([^$_]+?)(?=(?:\$|_)[a-z0-9]\s|$)/i);
    const b = bruto.match(/(?:\$|_)b\s*([^$_]+?)(?=(?:\$|_)[a-z0-9]\s|$)/i);

    if (a) {
      return limparTexto(a[1] + (b ? ' ' + b[1] : ''));
    }
    ...
    for (let i = 0; i < cells.length - 1; i++) {
      ...
      if (
        valor &&
        (label.includes('elemento de entrada') ||
         label.includes('entry element') ||
         label === 'a' ||
         label === '$a')
      ) {
        return valor;
  ...
  async function obterQID(authid) {
    ...
    for (const url of urls) {
      try {
        ...
        console.warn('AuthorityBox: erro ao obter QID', authid, url, e);
      }
    }
  ...
  function criarCaixa() {
    ...
    if (alvo.tagName && /^(UL|OL)$/i.test(alvo.parentElement && alvo.parentElement.tagName || '')) {
      const host = document.createElement('li');
      host.className = 'authoritybox-rbmo-intranet-host';
      host.style.listStyle = 'none';
      host.style.margin = '14px 0 0 0';
      host.style.padding = '0';
      host.appendChild(box);
      alvo.insertAdjacentElement('afterend', host);
      return;
    }
  ...
  function obterAlvoInsercao() {
    ...
    const relatorio = links.find(function (a) {
      return /relat[oó]rio de modifica/i.test(limparTexto(a.textContent));
    });

    if (relatorio) {
      return (
        relatorio.closest('li') ||
        relatorio.closest('.list-group-item') ||
        relatorio.closest('.sidebar_menu_item') ||
        relatorio.parentElement ||
        relatorio
      );
    }
...
(function () {
  ...
  function arrancarDescoberta() {
    if (!window.location.pathname.includes('/cgi-bin/koha/catalogue/detail.pl')) return;
  ...
  function obterAutorPrincipalDaPagina() {
    if (window.__authorityBoxRbmoPrimaryAuthor) {
      return window.__authorityBoxRbmoPrimaryAuthor;
    }
    ...
    for (const link of links) {
      const authid = extrairAuthid(link.getAttribute('href') || '');
      const nome = limpar(link.textContent);
      if (!authid || !nome) continue;
  ...
  function obterUrlPesquisaAutor(autor) {
    if (autor.authid) {
      return location.origin + '/cgi-bin/koha/catalogue/search.pl?idx=an,ext&q=' +
        encodeURIComponent(autor.authid) + '&count=50';
    }

    if (autor.nome) {
      return location.origin + '/cgi-bin/koha/catalogue/search.pl?idx=au&q=' +
  ...
  async function obterObras(url) {
    ...
    const links = Array.from(doc.querySelectorAll('a[href*="catalogue/detail.pl"][href*="biblionumber="]'));
    ...
    links.forEach(function (a) {
      ...
      mapa.set(bib, {
        biblionumber: bib,
        titulo: titulo,
        href: location.origin + '/cgi-bin/koha/catalogue/detail.pl?biblionumber=' + encodeURIComponent(bib),
        capa: capa,
